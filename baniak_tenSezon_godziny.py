...
workBook = load_workbook('/Users/przemyslaw.szafulski/Downloads/Sesje_Ten_Sezon.xlsx')
...
for nameCell in nameColumn:
    if (nameCell.value == None):
        continue
    if (shouldAddSession(nameCell.row) == False):
        continue
    found = False
    for person in people:
        if person.searchName in nameCell.value:
            ...
            if person.sessionLengthInSeconds < 100:
                print("Here")
    if found == False:
...
fig.update_layout(title_text='Ten Sezon BCU - Ilość Godzin', template="plotly_dark")
fig.write_html('/Users/przemyslaw.szafulski/Downloads/baniak_ten_sezon_godziny.html')